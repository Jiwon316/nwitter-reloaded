...
export default function Profile() {
  ...
  const onAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length === 1) {
      ...
      const locationRef = ref(
        storage,
        `users/${user.uid}-${user.displayName}/avatar`
      );
  ...
  const [edittedName, setName] = useState(user?.displayName || "Anonymous");
  const onUsernameEdit = () => {
    setEditName(true);
  };
  const onUernameChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setName(event.target.value);
  };
  const onUsernameSave = async()=>{
    if(!user || !edittedName)return;
    try {
      await updateProfile(user,{
        displayName: edittedName,
      });  
    } catch (error) {
      console.log(error);
    }finally{
